...
class DeploymentCard extends Component {
  ...
  componentDidMount() {
    eckApi
      .getDeploymentStatus(this.props.project, this.props.item.name)
      .then((response) => {
  ...
  render() {
    ...
    return (
      <EuiCard title={item.name} style={{ position: "relative" }}>
        ...
        <EuiFieldText
          value={"https://" + item.elasticsearch.Id + "." + item.base_url}
          prepend={[<EuiIcon type="logoElasticsearch" />]}
          readOnly={true}
        />
        <EuiSpacer size="s" />
        <EuiFieldText
          value={"https://" + item.kibana.Id + "." + item.base_url}
...
class Deployments extends Component {
  ...
  componentDidMount() {
    ...
    if (this.props.match.params.id !== undefined) {
      ...
      eckApi
        .getDeployments(this.props.match.params.id)
        .then((response) => {
      ...
      eckApi
        .getProjectQuota(this.props.match.params.id)
        .then((response) => {
          if (Array.isArray(response.data)) {
  ...
  render() {
    ...
    return (
      <EuiPageContent>
                    ...
                    <EuiButton
                      ...
                      onClick={() =>
                        this.props.history.push(
                          "/projects/" + this.props.match.params.id + "/create"
                        )
                      }
                    >
                      Create Deployment
                ...
                {Auth.hasProjectAdmin() && (
                  <EuiFlexItem grow={false}>
                    <EuiButton
                      ...
                      onClick={() =>
                        this.props.history.push(
                          "/projects/" + this.props.match.params.id + "/edit"
                        )
                      }
                    >
                      Edit Project
        ...
        <EuiPageContentBody>
          <div
            style={{ maxWidth: 400, marginTop: "-25px", marginBottom: "30px" }}
          >
            <EuiFlexGroup alignItems="center">
              <EuiFlexItem grow={2} style={{ margin: 7 }}>
                <EuiText size="xs">
                  <p style={{ textAlign: "right" }}>
                    CPU: {quota.Percentage.CPU * 100}%
                  </p>
                </EuiText>
              </EuiFlexItem>
              <EuiFlexItem grow={8} style={{ margin: 7 }}>
                <EuiProgress
                  value={quota.Percentage.CPU * 100}
                  max={100}
                  size="l"
                  color={quota.Percentage.CPUStatus}
                />
              </EuiFlexItem>
            </EuiFlexGroup>
            ...
            <EuiFlexGroup alignItems="center">
              <EuiFlexItem grow={2} style={{ margin: 7 }}>
                <EuiText size="xs">
                  <p style={{ textAlign: "right" }}>
                    Memory: {quota.Percentage.Memory * 100}%
                  </p>
                </EuiText>
              </EuiFlexItem>
              <EuiFlexItem grow={8} style={{ margin: 7 }}>
                <EuiProgress
                  value={quota.Percentage.Memory * 100}
                  max={100}
                  size="l"
                  color={quota.Percentage.MemoryStatus}
                />
              </EuiFlexItem>
            </EuiFlexGroup>
          </div>
