...
class DeploymentForm extends Component {
  ...
  componentDidMount() {
    ...
    if (this.props.match.params.id !== undefined && this.props.match.params.depoyment_id  ) {
      eckApi
        .getDeployment(
  ...
  onFormSave = () =>{
    ...
    deployment.elasticsearch.Config = JSON.stringify(this.state.elasticsearch.Config)
    ...
    if (!deployment.kibana.Enabled) {
      ...
      deployment.kibana.InstanceCount  = parseInt(deployment.kibana.InstanceCount, 10);
    }
  ...
  render() {
    return (
        <EuiPageContent>
            <EuiPageContentHeader>
                    <EuiPageContentHeaderSection>
                      <EuiTitle>
                        {this.props.match.params.id !== undefined && this.props.match.params.depoyment_id ? <h2>Deployment Edit</h2> : <h2>Deployment Create</h2>}
                      </EuiTitle>
                    </EuiPageContentHeaderSection>
                  </EuiPageContentHeader>
                  <EuiPageContentBody>
    ...
    <EuiFormRow
      ...
      <EuiFieldText fullWidth name="base_url" value={this.state.base_url} onChange={this.onTextChange}/>
    </EuiFormRow>

    <EuiFormRow
    label="Version"
    fullWidth>
     <EuiSelect fullWidth options={versions} value={this.state.Version}  onChange={this.onVersionChange}
...
{/*   
    ...
    {this.state.checked &&
      ...
      <EuiSpacer/>
<ElasticSearchForm data={this.state.elasticsearch} onConfigChange={this.onElasticChange}/>
<KibanaForm data={this.state.kibana} onConfigChange={this.onKibanaChange}/>
<EuiSpacer />
